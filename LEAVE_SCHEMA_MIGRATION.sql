-- Leave Management System Migration Script
-- This script creates the necessary tables for the leave management system
-- IMPORTANT: Uses accepted_schools table (not schools) to match your database schema
-- Make sure the accepted_schools table exists before running this script
--
-- NOTE: If you get "column specified more than once" error, it means the table
-- already exists. You can either:
-- 1. Drop the existing table first (if safe to do so)
-- 2. Or run only the sections that add missing columns
--
-- To check existing tables: SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name LIKE '%leave%';

-- ============================================================================
-- STEP 1: Create leave_types table
-- ============================================================================
...
-- ============================================================================
-- STEP 2: Create staff_leave_requests table (Complete Schema)
-- ============================================================================
-- Check if table exists, if not create it, if yes add missing columns
DO $$ 
BEGIN
  -- Create table if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'staff_leave_requests') THEN
    -- Use EXECUTE to avoid parsing issues
    EXECUTE '
...
-- Indexes for staff_leave_requests
...
-- ============================================================================
-- STEP 3: Create student_leave_requests table (Complete Schema)
-- ============================================================================
-- Check if table exists, if not create it
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'student_leave_requests') THEN
    CREATE TABLE student_leave_requests (
      id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
      school_code VARCHAR(50) NOT NULL REFERENCES accepted_schools(school_code) ON DELETE CASCADE,
      student_id UUID NOT NULL REFERENCES students(id) ON DELETE CASCADE,
      leave_type_id UUID NOT NULL REFERENCES leave_types(id) ON DELETE RESTRICT,
      leave_title VARCHAR(200), -- Optional title for the leave
      leave_applied_date TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
      leave_start_date DATE NOT NULL,
      leave_end_date DATE NOT NULL,
      reason TEXT NOT NULL, -- Required reason for leave
...
-- Indexes for student_leave_requests
...
CREATE POLICY "Users can view leave types for their school"
  ...
  USING (true); -- Allow all authenticated users to view (can be restricted further if needed)
...
-- 
-- Tables Created:
-- 1. leave_types - Stores different types of leaves (CL, EL, SL, etc.)
-- 2. staff_leave_requests - Stores leave requests from staff members
-- 3. student_leave_requests - Stores leave requests from students
--
-- Key Features:
-- - All tables reference accepted_schools(school_code) for foreign key
-- - staff_leave_requests includes both 'comment' and 'reason' fields
-- - student_leave_requests has required 'reason' field
-- - Automatic timestamp updates via triggers
-- - Row Level Security (RLS) enabled for all tables
-- - Views created for easy querying with joined data
--
-- ============================================================================
