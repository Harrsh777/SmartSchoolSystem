...
export default function BulkMarksUploadPage() {
  ...
  const [teachingAssignments, setTeachingAssignments] = useState<TeachingRow[]>([]);
  ...
  useEffect(() => {
    ...
    setTeacher(t);
    setSchoolCode(String(t.school_code || ''));
    void loadExams(t);
  }, []);

  const loadExams = async (t: Record<string, unknown>) => {
    try {
      setLoading(true);
      const q = new URLSearchParams({ school_code: String(t.school_code || '') });
      if (t.id) q.set('teacher_id', String(t.id));
      if (t.staff_id) q.set('staff_id', String(t.staff_id));
      const res = await fetch(`/api/examinations/v2/teacher?${q}`);
      const json = await res.json();
      if (res.ok && json.data) {
        setTeachingAssignments(json.teaching_assignments || []);
  ...
  const allowedClassIds = useMemo(() => new Set(teachingAssignments.map((a) => a.class_id)), [teachingAssignments]);

  const classOptions = useMemo(() => {
    if (!selectedExam?.class_mappings) return [];
    return selectedExam.class_mappings.filter(
      (cm) => allowedClassIds.size === 0 || allowedClassIds.has(cm.class_id)
    );
  }, [selectedExam, allowedClassIds]);
  ...
  const handleDownloadTemplate = async () => {
    if (!canDownload || !teacher?.id) return;
    setDownloading(true);
    setUploadResult(null);
    try {
  ...
  const handleUpload = async () => {
    if (!file || !schoolCode || !teacher?.id || !selectedExam || !selectedClassId || !selectedSubjectId) return;
    setUploading(true);
    setUploadResult(null);
    try {
      ...
      fd.set('entered_by', String(teacher.id));
  ...
  if (loading) {
    return (
      <div className="flex justify-center py-16 text-gray-600">Loading…</div>
    );
  }
  ...
  if (!teacher) {
    return (
      <Card className="p-6">
        <p className="text-gray-700">Please sign in as a teacher to use bulk upload.</p>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-4">
        <Link
          href="/teacher/dashboard/examinations/v2/marks-entry"
      ...
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Bulk upload marks</h1>
      ...
      {teachingAssignments.length > 0 && (
        <Card className="p-4">
          <h2 className="text-sm font-semibold text-gray-900 mb-2">Your timetable (classes & subjects)</h2>
          <ul className="text-sm text-gray-700 space-y-1">
            {teachingAssignments.map((a) => (
              <li key={a.class_id}>
                ...
                {a.subjects?.length ? (
                  <span className="text-gray-600"> — {a.subjects.map((s) => s.name).join(', ')}</span>
                ) : (
                  <span className="text-gray-500"> — {a.subject_ids.length} subject(s)</span>
                )}
              </li>
            ))}
          </ul>
        ...
        {selectedExam && (
          <div>
            ...
            {classOptions.length === 0 && (
              <p className="text-sm text-amber-700 mt-1">No class in this exam matches your timetable.</p>
            )}
          </div>
        )}
          ...
          {uploadResult.error && uploadResult.saved_count === undefined && (
            <div className="flex gap-2 text-red-800 bg-red-50 border border-red-200 rounded-lg p-4">
              ...
              <div>
                ...
                {uploadResult.code && (
                  <p className="text-sm mt-1 font-mono text-red-700">{uploadResult.code}</p>
                )}
              </div>
            </div>
          )}
