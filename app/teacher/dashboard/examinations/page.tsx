...
export default function TeacherExaminationsPage() {
  ...
  // Group exams by start date for date-wise view
  const examsByDate = useMemo(() => {
    const map = new Map<string, Exam[]>();
    for (const exam of sortedExams) {
      const key = exam.start_date
        ? new Date(exam.start_date).toISOString().slice(0, 10)
        : 'no-date';
        ...
        >
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400">View all school examinations date-wise</p>
            </div>
          </div>
        </motion.div>
          ...
          <Card>
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 dark:text-gray-500 text-sm mt-1">
                Examinations will appear here once they are created by the school.
              </p>
            </div>
          </Card>
        ) : (
          <div className="space-y-8">
            {examsByDate.map(({ dateKey, dateLabel, exams: dateExams }) => (
