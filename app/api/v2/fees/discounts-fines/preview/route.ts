...
import { classSectionCacheKey } from '@/lib/fees/class-fee-line-adjustments';
import { runBulkAdjustmentPreview, type BulkApplyBody } from '@/lib/fees/bulk-adjustment-apply';
import type { BulkChargeType, BulkValueMode } from '@/lib/fees/bulk-adjustment-math';

/**
 * POST /api/v2/fees/discounts-fines/preview
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runBulkAdjustmentPreview(supabase, applyBody, classLines);

    const okCount = result.rows.filter((r) => r.ok).length;
    const failCount = result.rows.length - okCount;

    return NextResponse.json({
      data: {
        structure_name: result.structure_name,
        rows: result.rows,
        summary: { total: result.rows.length, ok: okCount, failed: failCount },
        policy_note:
          'Multiple discounts on the same installment are additive (each line is stored separately). Percent discounts use base + rules + existing fines/charges.',
      },
      errors: result.errors,
    });
  } catch (e) {
    console.error('POST discounts-fines/preview', e);
