...
export async function GET(request: NextRequest) {
  try {
    ...
    const cacheKey = cacheKeys.attendanceOverview(schoolCode, classId, date, startDate, endDate);
    const response = await getCached(
      ...
      `)
        ...
        if (attendanceError) {
          ...
          if (simpleError) {
            throw new Error(`attendance_overview_failed:${simpleError.message}`);
          }
    ...
    (attendance || []).forEach((record: AttendanceRecord) => {
      ...
      { ttlSeconds: DASHBOARD_REDIS_TTL.attendanceOverview }
    );
