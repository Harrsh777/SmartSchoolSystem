...
export default function StudentDirectoryPage({
  ...
  const statusTabs: Array<{ id: StudentStatus; label: string; count: number }> = [
    ...
    { id: 'deactivated', label: 'Deactivated', count: students.filter(s => s.status === 'inactive' || s.status === 'deactivated').length },
    { id: 'transferred', label: 'Transferred', count: students.filter(s => s.status === 'transferred').length },
    { id: 'alumni', label: 'Alumni', count: students.filter(s => s.status === 'graduated' || s.status === 'alumni').length },
  ];

  const uniqueClasses = Array.from(new Set(students.map(s => s.class).filter(Boolean))).sort();
  const uniqueSections = selectedClass
    ? Array.from(new Set(students.filter(s => s.class === selectedClass).map(s => s.section).filter(Boolean))).sort()
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">#{student.admission_no || 'N/A'}</p>
                    </div>
                  </div>
                ...
                <div className="space-y-2 mb-4">
                  ...
                  {(student.father_contact || student.mother_contact) && (
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                      ...
                      <span>{student.father_contact || student.mother_contact}</span>
                    </div>
                  )}
                </div>
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-semibold text-gray-900">{student.student_name || 'N/A'}</div>
                            {student.email && (
                      ...
                      <td className="px-6 py-4 text-gray-700 font-medium">{student.admission_no || 'N/A'}</td>
                      <td className="px-6 py-4">
