...
export default function StaffLeavePage({
    ...
    try {
      ...
      if (response.ok && result.data) {
        ...
        if (allStaffResponse.ok && allStaffResult.data) {
          ...
          setStats({
            pending: allStaff.filter((l: StaffLeave) => l.status === 'pending').length,
            approved: allStaff.filter((l: StaffLeave) => l.status === 'approved').length,
            rejected: allStaff.filter((l: StaffLeave) => l.status === 'rejected').length,
          });
        }
      }
    } catch (err) {
  ...
  const handleRefresh = () => {
    setRefreshing(true);
    fetchStaffLeaves().finally(() => setRefreshing(false));
  };
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-1">Staff Leave</h1>
              <p className="text-gray-600 text-sm">Manage and approve staff leave requests</p>
            </div>
          </div>
          ...
          <div className="mb-6">
            <div className="relative">
              ...
              <Input
                ...
                placeholder="Search by staff name, ID, or leave type..."
