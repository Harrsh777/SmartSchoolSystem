...
- **#4 Rate limiting**: Implemented in `lib/rate-limit.ts`. Login endpoints (`/api/auth/login`, `/api/auth/student/login`, `/api/auth/teacher/login`, `/api/auth/accountant/login`) now enforce 10 requests per minute per IP. For multi-instance production, consider a shared store (e.g. Redis).
- **#9 Hardcoded URLs**: Added `lib/env.ts` with `getAppUrl()` / `getPublicBaseUrl()`. Marks export and certificate generation use it; `NEXT_PUBLIC_APP_URL` should be set in production. Fees configuration fallback and placeholder updated to use env or generic placeholder.
- **#12 Empty states**: Added reusable `components/ui/EmptyState.tsx`. Applied to Staff Management directory and Staff Attendance list views with clear messages and optional CTAs.
